  ...
  return async (req, res) => {
    ...
    try {
      switch (method) {
        ...
        case 'diff': {
          writeJson(res, 200, OK(await service.diff(cwd, payload.path, payload.staged === true)));
          return;
        }
        case 'stage': {
        ...
        case 'fetchAll': {
          writeJson(res, 200, OK(await service.fetchAll(cwd)));
          return;
        }
        case 'checkout': {
          writeJson(res, 200, OK(await service.checkout(cwd, { branch: String(payload.branch ?? '') })));
          return;
        }
        case 'createBranch': {
