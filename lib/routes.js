  ...
  return async (req, res) => {
    ...
    try {
      switch (method) {
        ...
        // TAG creation / local deletion: local git only (no credentials).
        case 'tagCreate': {
          writeJson(res, 200, OK(await service.tagCreate(subRepoDir(cwd, payload.repoPath), {
            tag: String(payload.tag ?? ''),
            target: String(payload.target ?? ''),
            message: String(payload.message ?? ''),
            annotate: payload.annotate === true,
            sign: payload.sign === true,
          })));
          return;
        }
        case 'tagDelete': {
          writeJson(res, 200, OK(await service.tagDelete(subRepoDir(cwd, payload.repoPath), {
            tag: String(payload.tag ?? ''),
          })));
          return;
        }
        ...
        case 'tagsRemote': {
          ...
          try {
            ...
            } else if (method === 'tagDeleteRemote') {
              out = await service.tagDeleteRemote(repoDir, {
                remote,
                tag: String(payload.tag ?? ''),
              }, opts);
            } else if (method === 'tagsFetch') {
              out = await service.tagsFetch(repoDir, {
                // '' means every configured remote (`fetch --all --tags`).
                remote: payload.remote === undefined || payload.remote === null ? '' : String(payload.remote),
              }, opts);
            } else if (method === 'tagsRemote') {
              out = await service.tagsRemote(repoDir, { remote }, opts);
            } else {
