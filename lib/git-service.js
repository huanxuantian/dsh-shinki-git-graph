...
/** Env for network git commands: allow terminal credential prompts (we feed
 *  them via stdin through `onPrompt`) and disable interactive SSH (fail fast
 *  so we can map auth errors to a friendly message instead of hanging). */
function netEnv(env = {}) {
  return {
    ...env,
