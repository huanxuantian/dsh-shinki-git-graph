...
/**
 * Resolve a session's working directory — HOST-side only, never from the
 * client. Three sources, in order of cost:
 *
 *  1. `ctx.sessions` — live sessions. This store is an in-memory map of the
 *     sessions currently OPEN in this instance (`get(id)` is a Map lookup),
 *     so it is empty for a workspace whose session is not attached yet;
 *  2. `sessionQuery.listSessions()` — the persisted session headers on disk,
 *     which cover any session regardless of open state;
 *  3. `workspaceRegistry.list()` — the workspace ledger, which maps a session
 *     id to its workspace path.
 *
 * Sources 2/3 are read through `ctx.get(...)` (allowed without an `inject`
 * declaration) so a profile without those services still degrades to the
 * pre-existing behaviour instead of failing to mount.
 */
async function lookupSessionCwd(ctx, sessionId) {
  const live = existingDir(ctx.sessions?.get?.(sessionId)?.header?.cwd);
  ...
  if (typeof query?.listSessions === 'function') {
    try {
      ...
      const persisted = existingDir(record?.header?.cwd ?? record?.cwd);
...
/** sessionId -> { cwd: string | null, at: number } */
const cwdCache = new Map();

/** Drop the resolved-cwd cache (test hook). */
export function resetSessionCwdCache() { cwdCache.clear(); }

/** Resolve the session's working directory (host authoritative — the client
 *  never supplies an arbitrary path). Returns null when no source yields an
 *  existing absolute directory. */
export async function sessionCwd(ctx, sessionId) {
  if (typeof sessionId !== 'string' || sessionId === '') return null;
  const cached = cwdCache.get(sessionId);
  if (cached !== undefined) {
    if (cached.cwd !== null || Date.now() - cached.at < CWD_MISS_TTL_MS) return cached.cwd;
    cwdCache.delete(sessionId);
  }
  const cwd = await lookupSessionCwd(ctx, sessionId);
