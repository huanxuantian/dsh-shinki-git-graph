...
export function createGitService(ctx) {
   ...
   *  configured (tracking relationship set). */
  async function checkout(cwd, { branch }) {
    if (!isValidBranchArg(branch)) throw new GitCommandError('非法分支名', E_BAD_REQUEST);
    const { local, remote, current } = await branches(cwd);
    if (branch === current) return { ok: true, unchanged: true };
    ...
    if (isRemoteRef) {
      ...
      // otherwise create it with --track so the upstream is configured.
      args = local.some((b) => b.name === localName)
