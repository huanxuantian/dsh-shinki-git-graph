...
export function createGitService(ctx) {
   ...
   *  the workspace's subdirectory relative to the repo root. For a fresh repo
   *  with no commits (unborn HEAD) the branch is reported as '' — there is no
   *  real branch yet, so the UI shows an empty branch tree rather than a
   *  misleading unborn branch name (e.g. the default 'main'). */
  ...
  async function branches(cwd) {
    ...
    // Unborn HEAD (fresh repo with no commits) is intentionally NOT listed:
    // for-each-ref has no branch ref to report, and surfacing the unborn
    // name (e.g. the default 'main') would falsely look like an existing
    // branch. A branch only appears once it has at least one commit.
