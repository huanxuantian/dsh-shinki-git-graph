...
/* 图谱列：上下两层 SVG —— 下层（纵向拉伸）画向下的线与扇出曲线，上层（1:1，含节点）
   画向上的线、汇入曲线与提交点；节点在上层末尾绘制，正好盖住下层的线头。
   max-width 限制图谱占行宽的比例，泳道极多时也不会把提交标题挤没。 */
.sgg-graph{position:relative;flex:none;align-self:stretch;min-height:28px;max-width:62%;overflow:hidden}
.sgg-graph-down{position:absolute;left:0;top:12px;width:100%;height:calc(100% - 12px)}
...
`;
    ...
    function GraphLanes({ lane, laneW, radius, branchColor, shape }) {
      ...
      return h('span', { className: 'sgg-graph', style: { width: `${width}px` } },
        ...
        h('svg', {
          ...
          width, height: NODE_Y, viewBox: `0 0 ${width} ${NODE_Y}`,
        }, up, node, ring),
      );
    }
    ...
    function GraphRow({ row, lane, laneW, radius, branch, open, onToggle, onContextMenu, branchColor, showTags }) {
    ...
    exports.__internals = { GraphRow, GraphLanes, layoutGraph, primPath, nodeShape, laneWidth, nodeRadius };
