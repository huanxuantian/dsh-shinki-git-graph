...
.sgg-dialog-run{background:var(--dsw-alias-brand-primary,#4d9fff);color:#fff}
    ...
    function GitGraphTab({ ctx, scope, visible, sidebarStore }) {
      ...
      const doStageAll = () => {
        if (subdir && !showAllRepo) {
          ...
          runWrite(async () => { for (const en of targets) await apiPost('stage', sessionId, { path: en.path }); });
        } else {
          doStage(undefined);
        }
      };
      const doUnstageAll = () => {
        if (subdir && !showAllRepo) {
          ...
          if (targets.length === 0) return;
          runWrite(async () => { for (const en of targets) await apiPost('unstage', sessionId, { path: en.path }); });
        } else {
          doUnstage(undefined);
        }
      };
      ...
      const doFetchAll = () => runWrite(() => apiPost('fetchAll', sessionId), t('fetchAllDone'));
      const openSync = async (kind) => {
        try {
          const [rs, ts] = await Promise.all([
            apiPost('remotes', sessionId),
            apiPost('tags', sessionId).catch(() => ({ tags: [] })),
          ]);
          ...
          showSyncDialog({
            ...
            onRun: (payload) => runWrite(async () => {
              ...
              const mainPromise = apiPost(kind, sessionId, { ...payload, opId });
              ...
              for (;;) {
                const p = await apiPost('prompt-poll', sessionId, { opId }).catch(() => ({ prompt: null }));
                if (p && p.prompt != null) {
                  const answer = await showPromptDialog(String(p.prompt));
                  await apiPost('prompt-answer', sessionId, { opId, value: answer === null ? '' : answer }).catch(() => {});
      ...
      const doCheckout = (branch, linkCurrent) => {
        if (busy) return;
        const run = () => runWrite(() => apiPost('checkout', sessionId, { branch, linkCurrent }), t('syncDone'));
        if (dirtyCount() > 0) showConfirm(t('checkout'), t('dirtyCheckoutMsg'), t('checkout'), run);
        else run();
      };
      const doCreateBranch = (name, base) => {
        if (busy) return;
        const run = () => runWrite(() => apiPost('createBranch', sessionId, { name, base }), t('syncDone'));
        if (dirtyCount() > 0) showConfirm(t('createBranch'), t('dirtyCreateMsg'), t('createBranch'), run);
        else run();
      };
      const doCheckoutCommit = (hash) => {
        if (busy) return;
        const run = () => runWrite(() => apiPost('checkoutCommit', sessionId, { hash }), t('syncDone'));
      ...
      const openWtInlineDiff = async (en, staged) => {
        ...
        try {
          ...
          const r = await apiPost('diff', sessionId, { path: en.path, staged, untracked });
