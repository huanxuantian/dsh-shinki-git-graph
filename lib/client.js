...
`;
    ...
    /** Main sidebar tab. */
    function GitGraphTab({ ctx, scope, visible }) {
      ...
      const doCheckout = (branch) => {
        if (busy) return;
        const run = () => runWrite(() => apiPost('checkout', sessionId, { branch }), t('syncDone'));
      ...
      const openCheckoutTo = () => showCheckoutDialog({
        ...
        remote: branches?.remote ?? [],
        currentBranch: branches?.current ?? '',
        onRun: (branch) => doCheckout(branch),
      });
