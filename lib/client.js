...
`;
    ...
    // Bounded auto-retry while no workspace session is available. A workspace
    // with no active session has NO cwd source: the sidebar scope carries only
    // sessionId (better-sidebar exposes no workspace id/path), so the panel can
    // never resolve a repository on its own. Polling must therefore stop and say
    // so, instead of refreshing forever (which is what users saw as "一直在反复刷新").
