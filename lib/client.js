...
.sgg-detail{border-top:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.1));max-height:33vh;display:flex;flex-direction:column;overflow:hidden;background:var(--dsw-alias-bg-layer-1,rgba(0,0,0,.15))}
.sgg-detail-fixed{flex:none;padding:8px 10px 4px;overflow-y:auto;max-height:45%}
.sgg-stat-wrap{flex:0 0 auto;min-height:0;max-height:calc(33vh * 0.25 + 3em);overflow-y:auto;padding:0 10px}
.sgg-detail-scroll{flex:0 1 auto;min-height:0;overflow-y:auto;padding:0 10px 8px}
...
`;
    ...
    function GraphRow({ row, lane, branch, open, onToggle, branchColor }) {
    ...
    function GitGraphTab({ ctx, scope, visible, onOpenDiff }) {
      ...
      if (phase === 'loading') body = h('div', { className: 'sgg-empty' }, t('loading'));
        ...
        detail && h(CommitDetail, { key: detail.hash, detail, branch: branches.current, onOpenDiff, onClose: () => setExpandedHash(null) }),
