...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     *  Cross-page continuation: pass the previous page's final lane state as
     *  `initialLanes`; the returned array carries a `tail` property with the
     *  final lane state for the following page. */
...
.sgg-commit-btn{flex:none;border:none;border-radius:6px;background:var(--dsw-alias-brand-primary,#4d9fff);color:var(--dsw-alias-label-primary-foreground,#fff);padding:4px 12px;font-size:12px;cursor:pointer}
...
.sgg-repo-branch{flex:none;font-size:10px;padding:0 5px;border-radius:8px;background:rgba(77,159,255,.16);color:#79c0ff}
...
.sgg-notice{padding:8px 12px;color:#7ee787;white-space:pre-wrap}
...
.sgg-dialog-select{flex:1;min-width:0;background:var(--dsw-alias-button-tool-bar-fill,transparent);color:var(--dsw-alias-label-primary,#e6edf3);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.1));border-radius:6px;padding:4px 6px;font-size:12px;outline:none}
.sgg-dialog-btns{display:flex;justify-content:flex-end;gap:8px}
.sgg-dialog-run{background:var(--dsw-alias-brand-primary,#4d9fff);color:var(--dsw-alias-label-primary-foreground,#fff)}
...
`;
    ...
    function showSyncDialog({ kind, remotes, local, remote, tags, upstreamOf, currentBranch, onRun, onFetchAll }) {
      ...
      runBtn.onclick = () => {
        ...
        if (isTag && tag === '') {
          ...
          tagInput.style.borderColor = '#ff7b72';
          tagInput.focus();
          return;
        }
        if (!isTag && branch === '') {
          branchSel.style.borderColor = '#ff7b72';
          return;
        }
        hideDialog();
    ...
    function showBranchDialog({ local, remote, currentBranch, defaultBase, onRun }) {
      ...
      runBtn.onclick = () => {
        ...
        if (name === '') {
          nameInput.style.borderColor = '#ff7b72';
          nameInput.focus();
          return;
        }
        hideDialog();
    ...
    function GitGraphTab({ ctx, scope, visible, sidebarStore }) {
      ...
      const api = (method, payload = {}, signal) =>
        apiPost(method, sessionId, activeRepoRef.current ? { ...payload, repoPath: activeRepoRef.current } : payload, signal);
      ...
      const loadRepoData = useCallback(async (repoPath) => {
        ...
        const [b, st] = await Promise.all([
          apiPost('branches', sessionId, p),
          apiPost('status', sessionId, p).catch(() => null),
        ]);
        setBranches(b);
        ...
        setRows(g.rows);
        const maps = assignLanes(g.rows, []);
        setLaneMaps(maps);
        lanesTailRef.current = maps.tail;
        setEnded(g.ended);
        setPhase('ready');
      }, [sessionId]);
      ...
      const load = useCallback(async () => {
        ...
        try {
          ...
          const init = await apiPost('init', sessionId, rp ? { repoPath: rp } : {});
          if (init.isRepo) {
      ...
      // While waiting for a workspace session (no-session), poll so the
      // graph appears automatically once a session becomes available.
      useEffect(() => {
        if (!visible || phase !== 'no-session') return;
        const timer = setInterval(() => { if (visible) load(); }, 2000);
      ...
      const doCommit = async () => {
        ...
        if (!msg || busy) return;
        await runWrite(() => api('wcommit', { message: msg }));
        setCommitMsg('');
        setStatusOpen(false);
      };
            ...
            onRun: (payload) => runWrite(async () => {
              ...
              for (;;) {
                ...
                if (p && p.prompt != null) {
                  ...
                  await api('prompt-answer', { opId, value: answer === null ? '' : answer }).catch(() => {});
      ...
      const doCreateBranch = (name, base) => {
        if (busy) return;
        const run = () => runWrite(() => api('createBranch', { name, base }), t('syncDone'));
      ...
      const loadMore = async () => {
        ...
        try {
          ...
          const g = await apiPost('graph', sessionId, { ...p, ...params, skip: rows.length, limit: pageSize });
          setRows((prev) => [...prev, ...g.rows]);
          // Continue lanes from the previous page's tail so the泳道 doesn't
          // break at the page boundary.
          const maps = assignLanes(g.rows, lanesTailRef.current);
          setLaneMaps((prev) => [...prev, ...maps]);
          lanesTailRef.current = maps.tail;
          setEnded(g.ended);
        } catch (err) { setError(err?.message ?? String(err)); }
      ...
      // Lane maps are maintained incrementally (page 1 + appended pages with
      ...
      if (phase === 'loading') body = h('div', { className: 'sgg-empty' }, t('loading'));
        ...
        t('noSession'), h('div', { className: 'sgg-hint' }, t('noSessionHint')));
