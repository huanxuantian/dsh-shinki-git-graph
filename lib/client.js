...
.sgg-wt{margin:2px 8px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.1));border-radius:6px;overflow:hidden;flex:none}
.sgg-wt-head{display:flex;align-items:center;gap:6px;padding:5px 8px;cursor:pointer;font-size:12px;color:var(--dsw-alias-label-primary,#e6edf3);background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.05))}
.sgg-wt-head:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.09))}
.sgg-wt-caret{flex:none;color:var(--dsw-alias-label-tertiary,#8b949e);width:12px}
.sgg-wt-title{font-weight:600}
.sgg-wt-body{padding:4px 8px 6px}
...
.sgg-commit-box{display:flex;align-items:center;gap:6px;padding:6px 8px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.08));flex:none}
...
`;
    ...
    function GitGraphTab({ ctx, scope, visible, onOpenDiff }) {
      ...
      const onWorkAction = (e) => {
        ...
        else if (v === 'commit') setCommitOpen(true);
      };
      ...
      if (phase === 'loading') body = h('div', { className: 'sgg-empty' }, t('loading'));
        ...
        commitOpen && h('div', { className: 'sgg-commit-box' },
          h('input', {
          ...
          h('button', { className: 'sgg-link', onClick: () => setCommitOpen(false) }, t('cancel')),
        ),
        h(BranchTree, { branches, scopeMode, selected, onToggleBranch: toggleBranch, currentBranch: branches.current, branchColors }),
        status && status.entries.length > 0 && h('div', { className: 'sgg-wt' },
          h('div', {
            ...
            h('span', { className: 'sgg-wt-title' }, `${t('uncommitted')} (${status.entries.length})`),
          ),
