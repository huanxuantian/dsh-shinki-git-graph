...
.sgg-confirm-msg{font-size:12px;line-height:1.6;color:var(--dsw-alias-label-secondary,#9da7b3);word-break:break-all;margin-bottom:14px}
...
`;
    ...
    function GitGraphTab({ ctx, scope, visible, onOpenDiff }) {
      ...
      const doDiscard = (path) => {
        ...
        showConfirm(
          isUntracked ? t('confirmDelete') : t('confirmDiscard'),
          `${isUntracked ? t('confirmDelUntracked') : t('confirmDiscardFile')} ${path}`,
          isUntracked ? t('confirmDelete') : t('confirmDiscard'),
      ...
      // One worktree file row: double-click opens the matching diff tab;
      // right-click offers stage/unstage/discard; staged vs unstaged use
      // distinct action icons (trash = unstage, branch = stage, like built-in).
      const openWtDiff = (en, staged) => {
        onOpenDiff?.({
          id: `diff:w:${staged ? 's' : 'u'}:${en.path}`,
          type: 'diff',
          title: en.path.split('/').pop(),
          diff: { kind: 'worktree', path: en.path, staged, untracked: en.xy === '??' },
        });
      };
      const fileRowMenu = (en, staged) => {
        ...
        if (staged) {
          ...
          if (en.xy !== '??') items.push({ label: t('discard'), onClick: () => doDiscard(en.path) });
        }
        return items;
      };
      const renderFileRow = (en, staged) => h('div', {
        key: en.path,
        className: `sgg-wt-file${en.xy === '??' ? ' sgg-wt-untracked' : ''}`,
        title: `${staged ? t('staged') : t('unstaged')} · ${t('viewDiff')}`,
        onDoubleClick: () => openWtDiff(en, staged),
        onContextMenu: (e) => {
          e.preventDefault();
          e.stopPropagation();
