...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      tagDeleteBtn: '删除 TAG',
      tagDeleteAlsoRemote: '同时删除对应远程 TAG',
    ...
    const en = {
      ...
      tagDeleteAlsoRemote: 'Also delete the matching remote tag',
...
.sgg-confirm-extra{display:flex;flex-direction:column;gap:8px;margin-bottom:12px}
...
`;
    ...
    function GitGraphTab({ ctx, scope, visible, sidebarStore }) {
      ...
      const doDeleteTag = (tagName, { remotes = [], defaultRemote = '' } = {}) => {
        ...
        showConfirm(
          ...
          hasRemote ? (box) => {
            // 附加（可选）：「同时删除远程 TAG」+ 远程选择 —— 不参与三重确认的按钮门控
            ...
            also.input.onchange = () => {
              extra.alsoRemote = also.input.checked;
              remoteRow.style.display = also.input.checked ? '' : 'none';
            };
            extra.remoteSel = sel;
    ...
    exports.__internals = { GraphRow, GraphLanes, ToggleIconButton, layoutGraph, primPath, nodeShape, laneWidth, nodeRadius, NODE_Y, ROW_H };
