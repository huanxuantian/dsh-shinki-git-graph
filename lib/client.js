 ...
 * computed by lib/graph-layout.js, which is inlined below (the browser half
 * cannot require sibling files; tests/client-inline.test.mjs guards the copy
 * and owns the inline-region helpers).
 */
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const primitives = {
      ...
      // 关闭态标签：标签牌 + 斜杠（用于「显示标签」开关，状态不靠颜色也能看出来）
      IconTagOffOutline16: (p) => _svg('M2.6 7.7V3.5a.9.9 0 0 1 .9-.9h4.2l5.7 5.7a1 1 0 0 1 0 1.4l-3.3 3.3a1 1 0 0 1-1.4 0L2.9 8.6a.9.9 0 0 1-.3-.9zM5.4 5.4h.01M2.2 13.8 13.8 2.2', p && p.size),
    };
    ...
    // 图谱布局/配色/曲线几何的唯一真源在 lib/graph-layout.js，逐字内联到下面这段
    // 标记区（浏览器半包不能 require 兄弟文件）。改算法请改真源并**同步更新本区**；
    // tests/client-inline.test.mjs 会守住两者逐字一致（内联区/标记名的定义也在该测试里）。
     ...
     *
     * 本文件**逐字内联**进 `lib/client.js` 的标记区（浏览器半包不能 require
     * 兄弟文件），由 `tests/client-inline.test.mjs` 守卫两者一致 —— 改这里就必须
     * 同步更新 client.js 的内联区（标记名/判定规则见该测试）。
    ...
    const zh = {
      ...
      stateOff: '关',
      // ── TAG（v0.10.0）──
      createTagHere: '在此创建 TAG',
      manageTags: '管理 TAG…',
      tagCreateTitle: '在此创建 TAG',
      tagName: 'TAG 名称',
      tagNamePlaceholder: '如 v1.2.0',
      tagTarget: '目标提交',
      tagAnnotate: '带注释（annotated，-a）',
      tagMessage: '注释消息',
      tagMessagePlaceholder: '标签说明（带注释/签名时必填）',
      tagSign: '带 GPG 签名（-s，需本机已配置密钥）',
      tagSignHint: '签名 TAG 必然带注释',
      tagAlsoPush: '同时推送到远程',
      tagPushTo: '推送到',
      tagNameRequired: '请填写 TAG 名称',
      tagMessageRequired: '带注释（或签名）的 TAG 必须填写注释消息',
      tagCreated: '已创建 TAG：{name}',
      tagCreatedPushed: '已创建并推送 TAG：{name} → {remote}',
      tagPushFailed: 'TAG {name} 已创建，但推送到 {remote} 失败：{message}',
      tagManageTitle: '管理 TAG',
      tagLocalList: '本地 TAG（{count}）',
      tagEmpty: '（没有本地 TAG）',
      tagAnnotatedBadge: '注释',
      tagLightweightBadge: '轻量',
      tagRemoteHas: '远程已有',
      tagRemoteMissing: '未推送',
      tagRemoteChecking: '正在读取远程 TAG…',
      tagRemoteUnknown: '远程状态未知',
      tagPushSelected: '推送选中 TAG',
      tagDeleteSelected: '删除选中 TAG',
      tagFetchRemote: '拉取远程 TAG',
      tagRefresh: '刷新',
      tagSelectFirst: '请先选择一个 TAG',
      tagPushed: '已推送 TAG：{name} → {remote}',
      tagFetched: '已拉取远程 TAG：{remote}',
      tagFetchedAll: '已拉取全部远程的 TAG',
      tagDeleted: '已删除本地 TAG：{name}',
      tagDeletedRemote: '已删除远程 TAG：{remote}/{name}',
      tagDeleteTitle: '删除 TAG',
      tagDeleteDanger: '危险操作：删除 TAG 不可撤销。删除后其他协作者仍可能按旧 TAG 拉取到不同内容，且本地删除不会自动同步到远程。',
      tagDeleteCheck1: '我确认要删除该 TAG',
      tagDeleteCheck2: '我明白删除后需要重新创建（注释/签名信息无法恢复）',
      tagDeleteCheck3: '我确认已核对 TAG 名称与目标提交',
      tagDeleteBtn: '删除 TAG',
      tagDeleteAlsoRemote: '同时删除远程 TAG',
      tagDeleteRemotePick: '删除远程',
      tagDeleteWontTouchRemote: '不勾选：只删本地 TAG，远程不受影响',
      tagDeleteRemoteHint: '只有远程上确实存在该 TAG 时才能删除；删除远程 TAG 需要网络与凭据。',
    };
    const en = {
      tabTitle: 'Git Graph',
      scope: 'Branches',
      ...
      tagCreateTitle: 'Create tag here',
      tagName: 'Tag name',
      tagNamePlaceholder: 'e.g. v1.2.0',
      tagTarget: 'Target commit',
      tagAnnotate: 'Annotated (-a)',
      tagMessage: 'Annotation message',
      tagMessagePlaceholder: 'Tag message (required when annotated/signed)',
      tagSign: 'GPG-signed (-s, needs a configured key)',
      tagSignHint: 'A signed tag is always annotated',
      tagAlsoPush: 'Also push to remote',
      tagPushTo: 'Push to',
      tagNameRequired: 'Enter a tag name',
      tagMessageRequired: 'An annotated (or signed) tag needs an annotation message',
      tagCreated: 'Tag created: {name}',
      tagCreatedPushed: 'Tag created and pushed: {name} → {remote}',
      tagPushFailed: 'Tag {name} was created, but pushing to {remote} failed: {message}',
      tagManageTitle: 'Manage tags',
      tagLocalList: 'Local tags ({count})',
      tagEmpty: '(no local tags)',
      tagAnnotatedBadge: 'annotated',
      tagLightweightBadge: 'lightweight',
      tagRemoteHas: 'on remote',
      tagRemoteMissing: 'not pushed',
      tagRemoteChecking: 'Reading remote tags…',
      tagRemoteUnknown: 'remote state unknown',
      tagPushSelected: 'Push selected tag',
      tagDeleteSelected: 'Delete selected tag',
      tagFetchRemote: 'Fetch remote tags',
      tagRefresh: 'Refresh',
      tagSelectFirst: 'Select a tag first',
      tagPushed: 'Tag pushed: {name} → {remote}',
      tagFetched: 'Remote tags fetched: {remote}',
      tagFetchedAll: 'Tags fetched from every remote',
      tagDeleted: 'Local tag deleted: {name}',
      tagDeletedRemote: 'Remote tag deleted: {remote}/{name}',
      tagDeleteTitle: 'Delete tag',
      tagDeleteDanger: 'DANGER: deleting a tag cannot be undone. Others may still fetch different content by the old tag, and a local delete does not touch the remote.',
      tagDeleteCheck1: 'I confirm I want to delete this tag',
      tagDeleteCheck2: 'I understand the tag must be recreated (annotation/signature cannot be restored)',
      tagDeleteCheck3: 'I confirm the tag name and target commit are correct',
      tagDeleteBtn: 'Delete tag',
      tagDeleteAlsoRemote: 'Also delete the remote tag',
      tagDeleteRemotePick: 'Remote',
      tagDeleteWontTouchRemote: 'Unchecked: only the local tag is deleted, the remote is untouched',
      tagDeleteRemoteHint: 'The remote tag can only be deleted when it actually exists there; deleting a remote tag needs network and credentials.',
    };
    const DICTS = { zh, en };
    const activeLocale = () => {
      try {
...
/* 开关的「开」态：主题色底 + 同色描边（用标签徽标那套 token，浅/深主题都明显；
   不要用 --dsw-alias-brand-primary —— 它在默认主题里是中性前景色，看不出状态）。 */
.sgg-iconbtn-on{background:var(--sgg-ref-bg);color:var(--sgg-ref-fg);box-shadow:inset 0 0 0 1px var(--sgg-ref-line)}
.sgg-iconbtn-on:hover{background:var(--sgg-ref-bg);color:var(--sgg-ref-fg)}
...
.sgg-confirm{position:fixed;z-index:2001;left:50%;top:40%;transform:translate(-50%,-50%);width:min(320px,calc(100vw - 40px));max-height:86vh;overflow-y:auto;border-radius:10px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));background:var(--dsw-alias-bg-overlay,#1b1d24);box-shadow:0 8px 24px var(--dsw-alias-bg-mask-2,rgba(0,0,0,.5));padding:14px 16px}
...
/* TAG 创建 / 管理对话框（v0.10.0）：宽版 + 只读值 + 多行注释 + TAG 列表 */
.sgg-dialog-wide{width:min(460px,calc(100vw - 40px))}
.sgg-dialog-value{flex:1;min-width:0;font-size:12px;color:var(--dsw-alias-label-primary,#e6edf3);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sgg-dialog-hint{font-size:10.5px;line-height:1.5;color:var(--dsw-alias-label-tertiary,#8b949e);margin:2px 0}
.sgg-dialog-error{color:var(--sgg-danger)}
.sgg-dialog-col{flex-direction:column;align-items:stretch;gap:4px}
.sgg-dialog-col .sgg-dialog-label{width:auto}
.sgg-dialog-textarea{width:100%;box-sizing:border-box;resize:vertical;font-size:11.5px}
.sgg-dialog-btns-left{display:flex;gap:8px;margin-bottom:10px}
.sgg-taglist-head{font-size:11px;color:var(--dsw-alias-label-secondary,#9da7b3);margin:8px 0 4px}
.sgg-taglist{max-height:38vh;overflow-y:auto;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.1));border-radius:6px}
.sgg-tagrow{display:flex;align-items:center;gap:6px;padding:4px 8px;cursor:pointer;font-size:11.5px;color:var(--dsw-alias-label-primary,#e6edf3);min-width:0}
.sgg-tagrow:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.05))}
.sgg-tagrow-sel{background:var(--sgg-ref-bg);box-shadow:inset 2px 0 0 var(--sgg-ref-fg)}
.sgg-tagname{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.sgg-tagkind{flex:none;font-size:9.5px;padding:0 5px;border-radius:8px;border:1px solid var(--sgg-ref-line);color:var(--sgg-ref-fg);background:var(--sgg-ref-bg)}
.sgg-tagkind-ann{border-color:var(--sgg-ref-tag-line);color:var(--sgg-ref-tag-fg);background:var(--sgg-ref-tag-bg)}
.sgg-tagoid{flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;color:var(--dsw-alias-label-tertiary,#8b949e)}
.sgg-tagmark{flex:none;font-size:9.5px;color:var(--dsw-alias-label-tertiary,#8b949e)}
.sgg-tagmark-on{color:var(--sgg-add)}
.sgg-confirm-wide{width:min(420px,calc(100vw - 40px))}
.sgg-confirm-extra{display:flex;flex-direction:column;gap:6px;margin-bottom:12px}
...
`;
    ...
    /** 开关型图标按钮。**状态必须能被看出来**，而且不能只靠颜色：
     *  · 图标切换（开/关两枚不同图标）；
     *  · 视觉（`.sgg-iconbtn-on`：主题色底 + 描边，随浅/深主题走）；
     *  · 无障碍与提示（`aria-pressed` + `title` 里带「开/关」）。
     *
     *  为什么专门抽出来：v0.9.1 实测「显示标签」按钮点了看不出有没有生效 ——
     *  ① 图标是 emoji `🏷`，**不继承 `color`**，设了高亮色也没变化；
     *  ② 高亮用的是 `--dsw-alias-brand-primary`，而它在默认主题里是**中性前景色**
     *  （深色近白、浅色近黑），与常态几乎同色。两点叠加就等于完全没有状态反馈。 */
    function ToggleIconButton({ on, title, IconOn, IconOff, onClick, size = 13 }) {
      const Icon = on ? IconOn : IconOff;
      return h('button', {
        type: 'button',
        className: `sgg-iconbtn${on ? ' sgg-iconbtn-on' : ''}`,
        title: `${title}（${on ? t('stateOn') : t('stateOff')}）`,
        'aria-label': title,
    ...
    function showConfirm(title, message, confirmLabel, onConfirm, items, buildExtra) {
      hideConfirm();
      const backdrop = document.createElement('div');
      ...
      // Optional extra controls (e.g. the tag-delete "also delete the remote
      // tag" checkbox + remote picker). They are NOT part of the checkbox gate
      // above: only the `items` checkboxes disable the confirm button.
      if (typeof buildExtra === 'function') {
        // ⚠ 这里必须用 appendChild：`insertBefore(extra, btns)` 会抛
        // NotFoundError（此刻 btns 还没挂到 box 上），而且异常会从按钮的 click
        // 处理器里冒出去 → 表现为「点了删除没反应、不弹窗」（v0.10.1 实测事故）。
        box.classList.add('sgg-confirm-wide');
        const extra = document.createElement('div');
        extra.className = 'sgg-confirm-extra';
        buildExtra(extra);
    ...
    /** Small DOM helpers for the native dialogs (keeps the builders readable). */
    function dlRow(cls = 'sgg-dialog-row') {
    ...
    function dlLabel(text) {
      ...
      el.className = 'sgg-dialog-label';
      el.textContent = text;
      return el;
    }
    function dlCheck(text) {
      const row = dlRow('sgg-dialog-row sgg-dialog-check');
    ...
    function dlSelect(options, value = '') {
      const sel = document.createElement('select');
      sel.className = 'sgg-dialog-select';
      ...
      if (value !== '' && options.some((o) => (o.value ?? o) === value)) sel.value = value;
      return sel;
    }

    /**
     * 「删除 TAG」确认框里的**可选**附加控件：「同时删除远程 TAG」+ 远程选择。
     * 刻意做成模块级纯 DOM 构建（可被 tests 直接驱动），并且：
     *   · 复选框**始终显示**（没有远程时置灰并给出说明），这样用户知道这个选项存在；
     *   · 远程下拉**始终显示**、勾选后才可操作（未勾选时置灰），默认选中上游远程/第一个远程；
     *   · 状态写回 `state.alsoRemote` / `state.remoteSel`，不参与三重确认的按钮门控。
     * @param {{remotes: Array<{name:string,url:string}>, defaultRemote?: string,
     *          state: {alsoRemote: boolean, remoteSel: any}}} opts
     * @returns {(box: HTMLElement) => void} 供 showConfirm 的 buildExtra 使用
     */
    function tagDeleteExtraControls({ remotes = [], defaultRemote = '', state }) {
      return (box) => {
        const hasRemote = remotes.length > 0;
        const also = dlCheck(t('tagDeleteAlsoRemote'));
        const remoteRow = dlRow();
        const sel = dlSelect(remotes.map((r) => ({ value: r.name, label: `${r.name}（${r.url}）` })), defaultRemote);
        remoteRow.appendChild(dlLabel(t('tagDeleteRemotePick')));
        remoteRow.appendChild(sel);
        const hint = document.createElement('div');
        hint.className = 'sgg-dialog-hint';
        hint.textContent = hasRemote ? t('tagDeleteRemoteHint') : t('noRemote');
        // 未勾选前不允许改远程（避免误以为不勾也会删远程）；没有远程则整体置灰
        sel.disabled = true;
        also.input.disabled = !hasRemote;
        also.input.onchange = () => {
          const on = also.input.checked && hasRemote;
          state.alsoRemote = on;
          sel.disabled = !on;
          hint.textContent = on ? t('tagDeleteRemoteHint') : (hasRemote ? t('tagDeleteWontTouchRemote') : t('noRemote'));
        };
        state.alsoRemote = false;
        state.remoteSel = sel;
        hint.textContent = hasRemote ? t('tagDeleteWontTouchRemote') : t('noRemote');
        box.appendChild(also.row);
        box.appendChild(remoteRow);
        box.appendChild(hint);
      };
    }

    /** 「在此创建 TAG」：名称 + 带注释（+ 消息）/ 带签名 + 可选「同时推送到远程」。
     *  创建走本地 git；勾了推送则在同一个写操作里接着走网络（带凭据桥）。 */
    function showCreateTagDialog({ target, targetText, remotes, defaultRemote, onRun }) {
      hideDialog();
      const backdrop = document.createElement('div');
      backdrop.className = 'sgg-dialog-backdrop';
      backdrop.onclick = hideDialog;
      ...
      const hd = document.createElement('div');
      hd.className = 'sgg-dialog-title';
      hd.textContent = t('tagCreateTitle');
      const form = document.createElement('div');
      form.className = 'sgg-dialog-form';

      // 目标提交（只读）
      const targetRow = dlRow();
      const targetVal = document.createElement('span');
      targetVal.className = 'sgg-dialog-value';
      targetVal.textContent = targetText;
      targetVal.title = target;
      targetRow.appendChild(dlLabel(t('tagTarget')));
      ...
      nameRow.appendChild(dlLabel(t('tagName')));
      nameRow.appendChild(nameInput);

      // 带注释 / 带签名
      const ann = dlCheck(t('tagAnnotate'));
      const sign = dlCheck(t('tagSign'));
      const signHint = document.createElement('div');
      signHint.className = 'sgg-dialog-hint';
      signHint.textContent = t('tagSignHint');

      // 注释消息（多行；勾注释/签名后必填）
      const msgRow = dlRow('sgg-dialog-row sgg-dialog-col');
      const msgLabel = dlLabel(t('tagMessage'));
      const msgInput = document.createElement('textarea');
      msgInput.className = 'sgg-commit-input sgg-dialog-textarea';
      msgInput.rows = 3;
      msgInput.placeholder = t('tagMessagePlaceholder');
      msgRow.appendChild(msgLabel);
      msgRow.appendChild(msgInput);

      // 同时推送到远程（+ 远程选择）
      const push = dlCheck(t('tagAlsoPush'));
      const pushRow = dlRow();
      const pushSel = dlSelect(remotes.map((r) => ({ value: r.name, label: `${r.name}（${r.url}）` })), defaultRemote);
      pushRow.appendChild(dlLabel(t('tagPushTo')));
      pushRow.appendChild(pushSel);
      const hasRemote = remotes.length > 0;
      push.input.disabled = !hasRemote;
      if (!hasRemote) push.row.title = t('noRemote');

      const syncAnnotate = () => {
        // -s 隐含 -a：勾了签名就自动勾注释并锁住（避免出现「签名但无注释」的无效组合）
        if (sign.input.checked) { ann.input.checked = true; ann.input.disabled = true; }
        else ann.input.disabled = false;
        const needMsg = ann.input.checked;
        msgRow.style.display = needMsg ? '' : 'none';
        if (needMsg) msgInput.focus();
      };
      ann.input.onchange = syncAnnotate;
      sign.input.onchange = syncAnnotate;
      push.input.onchange = () => { pushRow.style.display = push.input.checked ? '' : 'none'; };
      pushRow.style.display = 'none';
      syncAnnotate();

      form.appendChild(targetRow);
      form.appendChild(nameRow);
      form.appendChild(ann.row);
      form.appendChild(sign.row);
      form.appendChild(signHint);
      form.appendChild(msgRow);
      form.appendChild(push.row);
      ...
      runBtn.type = 'button';
      runBtn.className = 'sgg-confirm-btn sgg-dialog-run';
      runBtn.textContent = t('tagCreateTitle');
      runBtn.onclick = () => {
        const name = nameInput.value.trim();
        const annotate = ann.input.checked;
        const signed = sign.input.checked;
        ...
        if ((annotate || signed) && message === '') {
          msgInput.style.borderColor = 'var(--sgg-danger)';
          msgInput.focus();
          return;
        }
        const alsoPush = push.input.checked && hasRemote;
        hideDialog();
        onRun({ name, message, annotate, signed, alsoPush, remote: pushSel.value });
      };
      btns.appendChild(cancelBtn);
      btns.appendChild(runBtn);
      box.appendChild(hd);
    ...
    /** 「管理 TAG」：选一个 TAG 做推送/删除，拉取远程 TAG，并标出「远程已有」。
     *  删除是危险动作 → 三重确认，且可选同时删除对应远程 TAG（多远程时指定远程）。
     *  所有网络动作由调用方注入（它们要带凭据桥），失败时抛错由这里展示。 */
    function showManageTagsDialog({ remotes, defaultRemote, onLoadTags, onLoadRemoteTags, onPush, onDelete, onFetchRemote, onNotice }) {
      hideDialog();
      const hasRemote = remotes.length > 0;
      let tags = [];
      let selected = '';
      let remoteSet = null; // Set<string> | null（null = 读取中/不可知）
      let busy = false;
      ...
      backdrop.onclick = hideDialog;
      const box = document.createElement('div');
      box.className = 'sgg-dialog sgg-dialog-wide';
      const hd = document.createElement('div');
      hd.className = 'sgg-dialog-title';
      hd.textContent = t('tagManageTitle');

      const remoteRow = dlRow();
      const remoteSel = dlSelect(remotes.map((r) => ({ value: r.name, label: `${r.name}（${r.url}）` })), defaultRemote);
      remoteRow.appendChild(dlLabel(t('syncRemote')));
      remoteRow.appendChild(remoteSel);
      remoteRow.style.display = hasRemote ? '' : 'none';

      const listHead = document.createElement('div');
      listHead.className = 'sgg-taglist-head';
      const list = document.createElement('div');
      list.className = 'sgg-taglist';
      const hint = document.createElement('div');
      hint.className = 'sgg-dialog-hint';
      const err = document.createElement('div');
      err.className = 'sgg-dialog-hint sgg-dialog-error';

      // 远程 TAG 列表 → 每行标「远程已有 / 未推送」
      const loadRemote = async () => {
        remoteSet = null;
        render();
        if (!hasRemote) return;
        try {
          const names = await onLoadRemoteTags(remoteSel.value);
          remoteSet = new Set(names ?? []);
        } catch {
          remoteSet = null; // 未知：不显示标记，也不阻止操作（真正的错误在动作时给出）
        }
        render();
      };
      ...
      const act = async (fn, okText) => {
        ...
        try {
          ...
          tags = (await onLoadTags()) ?? tags;
          if (!tags.some((x) => x.name === selected)) selected = tags[0]?.name ?? '';
        } catch (e) {
      ...
      const render = () => {
        listHead.textContent = t('tagLocalList', { count: tags.length });
        list.innerHTML = '';
        if (tags.length === 0) {
          const empty = document.createElement('div');
          empty.className = 'sgg-dialog-hint';
          empty.textContent = t('tagEmpty');
          list.appendChild(empty);
        }
        for (const tg of tags) {
          const row = document.createElement('div');
          row.className = `sgg-tagrow${tg.name === selected ? ' sgg-tagrow-sel' : ''}`;
          row.tabIndex = 0;
          row.onclick = () => { selected = tg.name; render(); };
          row.onkeydown = (e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selected = tg.name; render(); } };
          const name = document.createElement('span');
          name.className = 'sgg-tagname';
          name.textContent = tg.name;
          name.title = tg.target ? `${tg.name} → ${tg.target}` : tg.name;
          const kind = document.createElement('span');
          kind.className = `sgg-tagkind${tg.annotated ? ' sgg-tagkind-ann' : ''}`;
          kind.textContent = tg.annotated ? t('tagAnnotatedBadge') : t('tagLightweightBadge');
          const oid = document.createElement('span');
          oid.className = 'sgg-tagoid';
          oid.textContent = String(tg.target ?? tg.oid ?? '').slice(0, 7);
          const mark = document.createElement('span');
          mark.className = 'sgg-tagmark';
          if (!hasRemote) mark.textContent = '';
          else if (remoteSet === null) mark.textContent = t('tagRemoteChecking');
          else if (remoteSet.has(tg.name)) { mark.textContent = t('tagRemoteHas'); mark.classList.add('sgg-tagmark-on'); }
          else mark.textContent = t('tagRemoteMissing');
        ...
        pushBtn.disabled = busy || !hasRemote || selected === '' || !tags.some((x) => x.name === selected);
        delBtn.disabled = busy || selected === '' || !tags.some((x) => x.name === selected);
        fetchBtn.disabled = busy || !hasRemote;
        refreshBtn.disabled = busy;
      };

      const tools = dlRow('sgg-dialog-btns-left');
      const refreshBtn = document.createElement('button');
      refreshBtn.type = 'button';
      refreshBtn.className = 'sgg-confirm-btn';
      refreshBtn.textContent = t('tagRefresh');
      refreshBtn.onclick = () => act(async () => { tags = (await onLoadTags()) ?? tags; await loadRemote(); });
      const fetchBtn = document.createElement('button');
      fetchBtn.type = 'button';
      fetchBtn.className = 'sgg-confirm-btn';
      fetchBtn.textContent = t('tagFetchRemote');
      fetchBtn.onclick = () => act(async () => {
        await onFetchRemote(remoteSel.value);
        onNotice?.(t('tagFetched', { remote: remoteSel.value }));
      });
      tools.appendChild(refreshBtn);
      tools.appendChild(fetchBtn);

      const actions = dlRow('sgg-dialog-btns-left');
      const pushBtn = document.createElement('button');
      pushBtn.type = 'button';
      pushBtn.className = 'sgg-confirm-btn sgg-dialog-run';
      pushBtn.textContent = t('tagPushSelected');
      pushBtn.onclick = () => act(async () => {
        const tag = selected;
        await onPush(tag, remoteSel.value);
        onNotice?.(t('tagPushed', { name: tag, remote: remoteSel.value }));
        await loadRemote();
      });
      const delBtn = document.createElement('button');
      delBtn.type = 'button';
      delBtn.className = 'sgg-confirm-btn sgg-confirm-danger';
      delBtn.textContent = t('tagDeleteSelected');
      delBtn.onclick = () => {
        if (selected === '') { err.textContent = t('tagSelectFirst'); return; }
        err.textContent = '';
        try {
          onDelete(selected, { remotes, defaultRemote: remoteSel.value });
        } catch (e) {
          // 打开确认框本身出错时也要有反馈（否则用户只看到「点了没反应」）
          err.textContent = e?.message ?? String(e);
        }
      };
      ...
      closeBtn.className = 'sgg-confirm-btn';
      ...
      box.appendChild(err);
      box.appendChild(btns);
      if (hasRemote) hint.textContent = t('tagDeleteRemoteHint');
      document.body.appendChild(backdrop);
      document.body.appendChild(box);

      render();
      (async () => {
        tags = (await onLoadTags()) ?? [];
    ...
    function GitGraphTab({ ctx, scope, visible, sidebarStore }) {
      ...
      /** 执行一个网络方法：先不 await 地发出去（git 可能阻塞在凭据提示上），
       *  同时轮询宿主的凭据桥 → 弹窗问答 → 再取结果。返回宿主 value，
       *  并顺带把「凭据是否被记住」的提示打出来。push 的 ahead/behind 提示由调用方处理。 */
      const apiNetwork = async (kind, payload) => {
        ...
        // `credential approve` on the configured helper after a successful
        // auth) — so all we do is tell the user what to expect next time.
        ...
        try {
          ...
          showSyncDialog({
            ...
            onRun: (payload) => runWrite(async () => {
              const result = await apiNetwork(kind, payload);
      ...
      // ── TAG（v0.10.0）：在此创建 / 管理（推送·删除·拉取远程）──
      /** 读远程 TAG 名（供管理面板标「远程已有」）；失败返回 [] 由面板显示未知。 */
      const loadRemoteTagNames = async (remote) => {
        const r = await apiNetwork('tagsRemote', { remote });
        return Array.isArray(r?.tags) ? r.tags.map((x) => x.name) : [];
      };
      /** 选中 TAG 的危险删除：三重确认；可同时删除对应远程 TAG（多远程时指定）。 */
      const doDeleteTag = (tagName, { remotes = [], defaultRemote = '' } = {}) => {
        if (busy) return;
        const extra = { alsoRemote: false, remoteSel: null, box: null };
        showConfirm(
          t('tagDeleteTitle'),
          `${t('tagDeleteDanger')}\n\n${tagName}`,
          t('tagDeleteBtn'),
          () => runWrite(async () => {
            await api('tagDelete', { tag: tagName });
            if (extra.alsoRemote && extra.remoteSel?.value) {
              await apiNetwork('tagDeleteRemote', { remote: extra.remoteSel.value, tag: tagName });
              setNotice(t('tagDeletedRemote', { remote: extra.remoteSel.value, name: tagName }));
              setTimeout(() => setNotice(''), 4000);
            } else {
              setNotice(t('tagDeleted', { name: tagName }));
              setTimeout(() => setNotice(''), 3000);
            }
          }),
          [
            { text: t('tagDeleteCheck1') },
            { text: t('tagDeleteCheck2') },
            { text: t('tagDeleteCheck3') },
          ],
          tagDeleteExtraControls({ remotes, defaultRemote, state: extra }),
        );
      };
      /** 「在此创建 TAG」：名称 / 注释 / 签名，可选创建后立即推送到指定远程。 */
      const openCreateTag = async (oid) => {
        try {
          const [rs, ts] = await Promise.all([
            api('remotes').catch(() => ({ remotes: [] })),
            api('tags').catch(() => ({ tags: [] })),
          ]);
          const remotes = Array.isArray(rs.remotes) ? rs.remotes : [];
          const row = rows.find((r) => r.oid === oid);
          const cur = branches?.current ?? '';
          const up = branches?.upstreamOf?.[cur] ?? '';
          const upstreamRemote = up.split('/')[0];
          const defaultRemote = remotes.some((r) => r.name === upstreamRemote)
            ? upstreamRemote : (remotes[0]?.name ?? '');
          const existing = new Set((ts.tags ?? []).map((x) => x.name));
          showCreateTagDialog({
            target: oid,
            targetText: `${oid.slice(0, 7)} ${row?.subject ?? ''}`.trim(),
            remotes,
            defaultRemote,
            onRun: ({ name, message, annotate, signed, alsoPush, remote }) => {
              if (existing.has(name)) {
                setError(t('tagCreated', { name }) + '：' + t('tagNameRequired'));
                return;
              }
              runWrite(async () => {
                await api('tagCreate', { tag: name, target: oid, message, annotate, sign: signed });
                if (!alsoPush) {
                  setNotice(t('tagCreated', { name }));
                  setTimeout(() => setNotice(''), 3000);
                  return;
                }
                // 本地已建好 → 推送失败时要说清楚「TAG 已创建」，别让用户以为整个操作失败
                try {
                  await apiNetwork('push', { remote, tag: name });
                  setNotice(t('tagCreatedPushed', { name, remote }));
                  setTimeout(() => setNotice(''), 4000);
                } catch (err) {
                  setError(t('tagPushFailed', { name, remote, message: err?.message ?? String(err) }));
                }
              });
            },
          });
      ...
      /** 「管理 TAG」：推送 / 危险删除（三重确认 + 可选删远程）/ 拉取远程 TAG。 */
      const openManageTags = async () => {
        try {
          const rs = await api('remotes').catch(() => ({ remotes: [] }));
          const remotes = Array.isArray(rs.remotes) ? rs.remotes : [];
          const cur = branches?.current ?? '';
          const upstreamRemote = (branches?.upstreamOf?.[cur] ?? '').split('/')[0];
          const defaultRemote = remotes.some((r) => r.name === upstreamRemote)
            ? upstreamRemote : (remotes[0]?.name ?? '');
          const readTags = async () => ((await api('tags')).tags ?? []);
          showManageTagsDialog({
            remotes,
            defaultRemote,
            onLoadTags: readTags,
            onLoadRemoteTags: loadRemoteTagNames,
            onPush: async (tag, remote) => { await apiNetwork('push', { remote, tag }); },
            onDelete: (tag, opts) => doDeleteTag(tag, opts),
            onFetchRemote: async (remote) => { await apiNetwork('tagsFetch', { remote }); },
          ...
          : h('div', { className: 'sgg-list' },
              h('div', { className: 'sgg-wt' },
                h('div', {
                  ...
                  h('span', { className: 'sgg-wt-actions' },
                    subdir && h(ToggleIconButton, {
                      on: showAllRepo,
                      title: t('showAllRepo'),
                      IconOn: primitives.IconFullscreenOutline16,
                      IconOff: primitives.IconFullscreenOutline16,
              ...
              rows.map((row, i) => h(GraphRow, {
                ...
                onContextMenu: (pos) => showCtxMenu(pos.x, pos.y, [
                  ...
                  { label: t('createTagHere'), onClick: () => openCreateTag(pos.row.oid) },
                  { label: t('manageTags'), onClick: () => openManageTags() },
      ...
      return h('div', { 'data-dsh-plugin': 'dsh-shinki-git-graph', style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, overflow: 'hidden' } },
        h('div', { className: 'sgg-header' },
          ...
          h(ToggleIconButton, {
            on: showTags,
            title: t('showTags'),
            IconOn: primitives.IconTagOutline16,
            IconOff: primitives.IconTagOffOutline16,
            size: 14,
    ...
    exports.__internals = { GraphRow, GraphLanes, ToggleIconButton, tagDeleteExtraControls, showConfirm, showManageTagsDialog, layoutGraph, primPath, nodeShape, laneWidth, nodeRadius, NODE_Y, ROW_H };
