...
`;
    ...
    function showSyncDialog({ kind, remotes, local, remote, tags, upstreamOf, currentBranch, onRun, onFetchAll }) {
      ...
      // Pull: "fetch only" (no merge / checkout) option — branch mode only;
      // a tag pull is inherently a fetch (never merges/checks out).
      let fetchCheck = null;
      let fetchRow = null;
      if (kind === 'pull') {
        ...
        const syncFetchVisibility = () => { fetchRow.style.display = typeSel.value === 'tag' ? 'none' : ''; };
      ...
      runBtn.onclick = () => {
        ...
        hideDialog();
        onRun({ remote, branch, tag, fetchOnly });
      };
      btns.appendChild(cancelBtn);
      ...
      const openSync = async (kind) => {
        try {
          ...
          showSyncDialog({
            ...
            onRun: (payload) => runWrite(() => apiPost(kind, sessionId, payload), t('syncDone')),
