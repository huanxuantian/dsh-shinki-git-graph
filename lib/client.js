...
`;
    ...
    function GitGraphTab({ ctx, scope, visible, onOpenDiff }) {
      ...
      const [ctxMenu, setCtxMenu] = useState(null); // {x, y, items:[{label,onClick}]}
      ...
      if (phase === 'loading') body = h('div', { className: 'sgg-empty' }, t('loading'));
        ...
        ctxMenu && h(Fragment, null,
          h('div', {
            className: 'sgg-menu-backdrop',
          ...
          h('div', {
            ...
            style: {
              ...
              top: Math.min(ctxMenu.y, window.innerHeight - 34 * ctxMenu.items.length - 8),
            },
          },
            ctxMenu.items.map((item) => h('button', {
              key: item.label,
              type: 'button',
              className: 'sgg-menu-item',
